...
logical :: is_even
    ...
    bin_loop: do ibin=1,nbin
        ...
        is_even = .false.
        if(mod(npt_per_bin(ibin),2)==0) then
            is_even = .true.
        end if
        if(is_even) then
            ipt_max = npt_per_bin(ibin)/2
        else
            ipt_max = (npt_per_bin(ibin)-1)/2
        end if
        points_loop: do ipt=1,ipt_max
            r = rand_num()
            jpt = jpt + 1
            rand_vec(jpt) = xmin + ((xmax-xmin)*r)
            !--antithetic pair--!
            jpt = jpt + 1
            rand_vec(jpt) = xmin + ((xmax-xmin)*(1.0e0-r))
        end do points_loop
        if(.not. is_even) then
            r = rand_num()
            jpt = jpt + 1
